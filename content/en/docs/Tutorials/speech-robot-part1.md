...
Part I will focus on recognizing speech commands through a microphone. [Part II]({{< relref "/docs/Tutorials/speech-robot-part2" >}}) will focus on translating commands into robot behavior and integrating with the mechanical platform.
...
In this part of the tutorial we will focus on building the sensor pipeline. In [part II]({{< relref "/docs/Tutorials/speech-robot-part2" >}}) we will take on building the state machine plus assembling and wiring it all together.
...
In [part II]({{< relref "/docs/Tutorials/speech-robot-part2" >}}) we will learn how to design the state machine component of the robot and how to interface with motor drivers. Then we will switch from bits to atoms and integrate the Arty board and PMOD modules with the chassis, motors and on-board power distribution.